# Policy for Solution V2 — Task ID action space (score ~70%)
...
class ChildPolicy(Policy):
    def __init__(self, model_title, seed=42):
        ...
        self.model = None
        self.seed = seed
        self.model_path = os.path.join("app", "data", "models", "ppo_second")
    ...
    def learn(self, scenario, total_timesteps, verbose=1, callback=None, **kwargs):
        print(f"--- [V2] Starting Training for {total_timesteps} steps (seed={self.seed}) ---")
        ...
        print(f"--- [V2] Training Finished. Model saved at {self.model_path}.zip ---")
