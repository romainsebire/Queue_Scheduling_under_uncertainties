# Policy for Solution V4 — 4 Semantic Strategies (score ~92.52%) — BEST
# Uses decaying learning rate and custom [128, 64] network architecture
...
class ChildPolicy(Policy):
    def __init__(self, model_title, seed=42):
        ...
        self.model = None
        self.seed = seed
        self.model_path = os.path.join("app", "data", "models", "ppo_fourth")
    ...
    def learn(self, scenario, total_timesteps, verbose=1, callback=None, **kwargs):
        print(f"--- [V4] Starting Training for {total_timesteps} steps (seed={self.seed}) ---")
        ...
        self.model = MaskablePPO(
            ...
            learning_rate=lambda p: 3e-4 * p,  # decaying LR
            n_steps=4096,
            batch_size=128,
            n_epochs=5,
            gamma=0.99,
        ...
        print(f"--- [V4] Training Finished. Model saved at {self.model_path}.zip ---")
