# Solution V1 — Action Space: Patient ID (score ~60%)
# Action = index of the patient in the queue (up to 50) + hold action
# Observation = Dict with queue matrix, servers matrix, context vector
...
class ChildEnv(Env):
    ...
    def _get_action_space(self):
        # 0..max_obs_queue-1 : choose customer at that index; max_obs_queue : hold
    ...
    def _get_observation_space(self):
        ...
        context_space = spaces.Box(low=0, high=np.inf, shape=(2,), dtype=np.float32)
        return spaces.Dict({"queue": queue_space, "servers": server_space, "context": context_space})
    ...
    def _get_obs(self):
        ...
        for i, customer in enumerate(sorted_customers):
            ...
            queue_matrix[i] = [wait_time, float(customer.task) / self.num_needs,
                               is_appt, appt_delta, est_service, time_before_abandon]
        ...
        context = np.array([sim_time / self.max_sim_time,
                            float(selected_server_id) / self.c], dtype=np.float32)
        return {"queue": queue_matrix, "servers": server_matrix, "context": context}
    ...
    def action_masks(self):
        ...
        for i in range(len(self.queue_mapping)):
            ...
            if customer and current_server.avg_service_time.get(customer.task, 0) > 0:
                mask[i] = True
                can_serve_someone = True
        mask[self._get_hold_action_number()] = not can_serve_someone
        return mask
