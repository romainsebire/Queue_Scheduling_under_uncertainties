...
class ChildEnv(Env):
    def __init__(self, mode, instance=None, scenario=None):
        # define parameters before calling super because action space is needed immediately
    ...
    def _get_customer_from_action(self, action):
        ...
        # case 2: patient with an appointment
            ...
            for c in self.customer_waiting.values():
                if c.id in self.appointments:
                    ...
                    if self.current_working_server.avg_service_time.get(c.task, 0) > 0:
                        ...
                        # 2. filter candidates (within time window)
        ...
        # greedy sort: fifo (minimize wait time)
    ...
    def action_masks(self):
        ...
        # 3. wait
        # anti-strike logic: work if possible
        if any(mask[:-1]): 
